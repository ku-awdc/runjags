...
This should verify that your JAGS installation has been found.  The rjags and modeest packages are not strictly required, but are recommended packages and should be installed where possible.
...
This function call takes an additional 5000 samples and combines them with the original simulation.  It is also possible to automatically extend a model until it has converged (as assessed by Gelman and Rubin's convergence diagnostic) using the autorun.jags and autoextend.jags functions, although manual inspection of trace plots is always recommended to ensure chains have really converged.  Note also that each of the functions run.jags, extend.jags, autorun.jags and autoextend.jags allows a method argument to be specified, which allows separate chains to be run on parallel processors where available.  Full details are provided are in the user guide, but the most used options are 'rjags', 'interruptible', 'parallel' and 'background'.  
...
The Deviance Information Criterion (DIC) is typically used to compare models fit using JAGS and BUGS, and is displayed as part of the print method shown above.  This can also be extracted from a runjags model using the extract method, along with other information such as the samplers used:
...
This package is in constant development, so if you find any issues or the code does not behave as expected please either email the package developer or visit the [bug-tracker page](https://github.com/ku-awdc/runjags/issues) to report the problem.  General comments and suggestions on how you find using this package are also very welcome.


## Citation

Development of this R package has consumed a great deal of time and energy, and is a somewhat peripheral activity in my current academic position.  In order to justify continued development of the software I need to be able to demonstrate that it is being used within the general research community.  So, if you use this software as part of a publication, please remember to cite the software as follows:
