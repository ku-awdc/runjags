...
combine.mcmc <- function(mcmc.objects=list(), thin=1, return.samples=NA, collapse.chains=NA, vars=NA, add.mutate=TRUE){
	...
	if(returnmcmcs && collapse.chains && nchain(mcmc.objects[[1]])==1){
	    ## TODO: reinstate warning but fix code that calls collapse.chains == TRUE when only a single chain is used
