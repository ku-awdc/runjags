...
#		}
		...
		if(strmethod=="snow"){
			...
			method <- runjags.snow
			rjt <- remote.jags
			if(inherits(rjt, "function")){
				s <- try(rjt <- rjt())
				if(inherits(s, "try-error")) stop("The function supplied to remote.jags must take no arguments")
			}
		...
		if(!inherits(method.options, "list") || any(names(method.options)=="")) stop("The method.options argument must be provided as a named list")
