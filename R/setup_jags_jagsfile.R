setup_jags <- function(model, monitor = stop("No monitored variables supplied"), data=NA,  n.chains=2, inits = replicate(n.chains, NA), modules=c(""), factories=c(""), response=NA, fitted =NA, residual=NA, jags = runjags.getOption('jagspath'), method="simple", mutate=NA){
  ...
  # Convert specified character to lists - or '' if none:
	...
	if(!inherits(model, "character") | length(model)!=1){
...
setup_jagsfile <- function(model, n.chains=NA, data=NA, inits=NA, monitor=NA, modules=c(""), factories=c(""), mutate=NA, jags=runjags.getOption('jagspath'), method="simple", call.setup=TRUE, failincomplete=TRUE){
	...
	if(inherits(model, "runjagsmodel"))
		...
		if(identical(mainmutate, NA)){
			...
			if(!identical(mutate, NA)){
				...
				if(inherits(s, 'try-error')){
					...
					if(!inherits(mutate, c('character','list','function'))){
	...
	if(!identical(funtoget, NA) && !is.function(funtoget)){
		...
		if(inherits(s, 'try-error') || !is.function(funfound))
...
check_data_format <- function(arg, block, auto, n.chains=NA, data.type=TRUE, evalscope=NULL){
	...
	if(inherits(arg, 'function')){
		if(data.type){
			...
			stopifnot(is.list(evalscope) && length(evalscope)==1)
			...
			if(inherits(s, "try-error")) stop(paste("The following error was encountered using the function supplied for 'inits':  ", s[1], sep=''), call.=FALSE)
		}
	}
